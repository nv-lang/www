...
Серия публикаций на Habr и других площадках требует **CTA в email-лист**, не только ссылку на репо. Рост email-листа — главный KPI: лист переносим (не зависит от платформы), компаундируется через все будущие запуски, конвертирует в 5–20× выше чем прямые ссылки на платный продукт. Без лендинга CTA некуда вести.
...
- Сам контент курса / newsletter (вне scope этого репо).
- Payment processing (это Boosty / GH Sponsors / ЮKassa напрямую, не наша инфра).
- Tier-схема для /sponsors/ (не вводим без 3+ реальных спонсоров — пустой прайс-лист = cargo cult).
...
Cloudflare Worker — отдельно от www репо (приватная инфраструктура). Содержит API-keys в env, реквесты с .org/.ru фильтруются через CORS.
...
- **Ф.0.1** Финальные тексты партиалов EN + RU. Контент:
  - **Course:** название, обещание (3-5 пунктов), целевая аудитория (middle/senior devs/teamleads), дата первого потока («Q1 2027» placeholder), цена (в плане не публикуется — отдельный внутренний параметр), форма waitlist.
...
- **Ф.1.1** Создать Worker (приватная инфраструктура — отдельный приватный репо или подкаталог в приватных проектных доках). Содержит:
...
- **Ф.5.3** **НЕ добавлять `/sponsors/` в навигацию** до релиза 0.1. Добавляется отдельным PR после релиза.
...
- Контент курса (вне scope этого репо).
...
- **Q2: Boosty placeholder text** — «настройка в процессе» (нейтрально) vs «откроется через N недель» (даёт expectation)? **Решение:** нейтрально — Boosty настраивается в рамках внутреннего плана, точная дата не критична.
...
- Blog-посты публикуются как PR в `site/src/pages/blog/` этого же репо;
  canonical для всех публикаций (включая внешние репосты на Habr / dev.to) —
  `nv-lang.org/blog/{slug}`.
